...
if is_vim then require "flannel.lib.vim" end
...
local M = {
	default_options = {
		...
		compile_path = vim.fn.stdpath "cache" .. "/flannel",
...
function M.compile()
	...
	for flavour, _ in pairs(M.flavours) do
		M.flavour = flavour
		require("flannel.lib." .. (is_vim and "vim." or "") .. "compiler").compile(flavour)
	end
...
local function get_flavour(default)
	local flavour
	if default and default == M.flavour and vim.o.background ~= (M.flavour == "muted" and "light" or "dark") then
	...
	if flavour and not M.flavours[flavour] then
		vim.notify(
			string.format(
				"Flannel (error): Invalid flavour '%s', flavour must be 'muted' or 'vibrant'",
				flavour
			),
	...
	return flavour or M.options.flavour or vim.g.flannel_flavour or M.options.background[vim.o.background]
end
...
---@type fun(user_conf: FlannelOptions?)
function M.setup(user_conf)
	...
	local hash = require("flannel.lib.hashing").hash(user_conf)
...
vim.api.nvim_create_user_command(
	"Flannel",
	function(inp) vim.api.nvim_command("colorscheme flannel-" .. get_flavour(inp.args)) end,
	{
		nargs = 1,
...
vim.api.nvim_create_user_command("FlannelCompile", function()
	for name, _ in pairs(package.loaded) do
		if name:match "^flannel." then package.loaded[name] = nil end
	end
	M.compile()
	vim.notify("Flannel (info): compiled cache!", vim.log.levels.INFO)
	vim.cmd.colorscheme "flannel"
end, {})

if vim.g.flannel_debug then
	vim.api.nvim_create_autocmd("BufWritePost", {
		pattern = "*/flannel/*",
		callback = function()
			vim.schedule(function() vim.cmd "FlannelCompile" end)
		end,
	})
end
