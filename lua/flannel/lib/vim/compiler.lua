local path_sep = require("flannel").path_sep
...
function M.compile(flavour)
	local theme = require("flannel.lib.mapper").apply(flavour)
	local lines = {
		string.format(
			[=[
...
let g:colors_name = "flannel-%s"]=],
	...
	if vim.g.flannel_debug then -- Debugging purpose
	...
	if not f then
		local err_path = (path_sep == "/" and "/tmp" or os.getenv "TMP") .. "/flannel_error.lua"
		print(string.format(
			[[Flannel (error): Most likely some mistake made in your flannel config
