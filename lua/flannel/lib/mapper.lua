...
function M.apply(flavour)
	flavour = flavour or require("flannel").flavour

	local _O, _C, _U = O, C, U -- Borrowing global var (setfenv doesn't work with require)
	O = require("flannel").options
	C = require("flannel.palettes").get_palette(flavour)
	U = require "flannel.utils.colors"
	...
	local theme = {}
	theme.syntax = require("flannel.groups.syntax").get()
	theme.editor = require("flannel.groups.editor").get()
	local final_integrations = {}

	-- https://github.com/flannel/nvim/pull/624
	...
	for integration in pairs(O.integrations) do
		...
		if type(O.integrations[integration]) == "table" then
			...
			if O.integrations[integration] == true then
				O.integrations[integration] = require("flannel").default_options[integration] or {}
		...
		if cot then
			final_integrations = vim.tbl_deep_extend(
				"force",
				final_integrations,
				require("flannel.groups.integrations." .. integration).get()
			)
		end
	end

	theme.integrations = final_integrations -- plugins
	theme.terminal = require("flannel.groups.terminal").get() -- terminal colors
